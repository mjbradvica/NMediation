// <copyright file="BasePostProcessingHandler.cs" company="Simplex Software LLC">
...
{
    /// <inheritdoc />
    public abstract class BasePostProcessingHandler<TPayload, TResponse> : IPayloadHandler<TPayload, TResponse>
        where TPayload : IPayload<TResponse>
    {
        /// <inheritdoc/>
        public async Task<TResponse> Handle(TPayload payload, CancellationToken cancellationToken)
        {
            var response = await DoWork(payload, cancellationToken);
